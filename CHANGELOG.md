...
- **Phase 7**: Background Workers, Automix Engine & Web MIDI
  - Added `src/audio/analysis/fft.ts` — iterative radix-2 Cooley-Tukey FFT (`fft()`, `nextPowerOfTwo()`)
  - Added `src/audio/analysis/bpmDetection.ts` — autocorrelation BPM detection over a half-wave-rectified onset envelope (`computeOnsetEnvelope`/`detectBpmFromOnsetEnvelope`/`detectBpm`)
  - Added `src/audio/analysis/keyDetection.ts` — Krumhansl-Schmuckler key-finding (chroma extraction from FFT magnitudes, Pearson correlation against 24 rotated major/minor profiles), mapped to Camelot wheel notation (`detectKey`/`toCamelotNotation`)
  - Added `src/audio/analysis/pcm.ts` — `monoDownmix()` for feeding a decoded `AudioBuffer` into the above
  - Added `src/workers/audioAnalysis.worker.ts` — background BPM/key/peak analysis run off the main thread via a throwaway `OfflineAudioContext`, wired through `src/services/trackAnalysis.ts`'s `analyzeTrackInBackground()`, which patches the result into the loading deck's store via a new `setTrackAnalysis(filePath, bpm, key)` action (guarded by `filePath` so a stale result from a since-replaced track is dropped rather than silently overwriting the new one)
  - `contextMenuTargets.ts`'s `loadTrackToDeck()` now kicks off `analyzeTrackInBackground()` alongside the existing decode+load, using the raw bytes `trackLoader.ts`'s `decodeTrack()` already read (no redundant re-read)
  - Added `src/audio/nodes/echoEffect.ts` — delay-line + feedback echo (`DelayNode` + feedback `GainNode` + wet/dry mix), defaulting to fully bypassed so it can sit permanently in a deck's signal chain; wired into `deckEngine.ts` between the volume fader and the deck's output, exposing `setEchoWetLevel()`
  - Added `src/audio/automixController.ts` — the ping-pong automix state machine: polls the active deck's position against `useAutomixStore`'s `triggerSecondsBeforeEnd`, loads the next queue entry into the idle deck, and runs one of the 4 transition styles (Smooth/Bass Swap/Cut/Echo Out) as a crossfader-position curve over `transitionSeconds`, plus a Bass Swap low-EQ kill swap or an Echo Out wet-level ramp as needed; also consumes Phase 3.5's `silenceWatchdog` for the dead-air fallback (an immediate hard cut to the next queued track, bypassing the graceful transition since there's no point crossfading into silence). Mounted once via `initAutomixController()` in `App.tsx`, matching the existing `initMidiHotPlug`/`initNativeFileDropListener` one-shot-init pattern
  - Added bidirectional MIDI LED feedback for the DDJ-400: `src/services/midi/ddj400Leds.ts` (pure state → LED-message mapping: play/cue LEDs from playback state, hot-cue pad LEDs from the deck's hot cues, sampler pad LEDs from the shared sampler bank) and `src/services/midiOutput.ts` (the `MIDIOutput.send()` glue, subscribed to the deck/sampler stores); `midiAccess.ts` extended to attach/detach outputs alongside its existing input hot-plug handling and start the LED sync once access is granted
  - Added 40 new tests (165 total): FFT/BPM/key-detection/PCM analysis tests, `echoEffect` bypass/wet/feedback/clamping tests, `automixController`'s pure transition-curve/bass-swap functions, and `ddj400Leds`' pure LED-mapping functions
  - Verified: `npx tsc --noEmit`, `npm run lint`, `npm run format:check`, `npm run test`, `cargo test --manifest-path src-tauri/Cargo.toml`, and `npm run tauri dev` all pass

- **Phase 8**: Radio, Karaoke, Themes & Release Packaging
  - Added `src-tauri/src/radio.rs` — a hand-rolled local TCP proxy on `127.0.0.1:17420` (`std::net` only, no HTTP client crate) that fetches a tuned Icecast/Shoutcast station itself, strips the interleaved ICY metadata blocks before forwarding audio bytes to the client (WebView2's `<audio>` element has no support for that interleaving), and emits a `radio://title-changed` Tauri event with the extracted "now playing" title. `http://` upstreams only — documented as a known limitation rather than adding a TLS crate for `https://`. Exposes `tune_radio_station(url)`, started once at app boot via `lib.rs`'s `.setup()` hook
  - Added `src-tauri/src/commands/lyrics.rs` — a minimal ID3v2 USLT (unsynchronized lyrics) frame reader (`read_embedded_lyrics`), handling v2.3 frame sizes correctly; v2.4's syncsafe frame sizes are a documented known limitation (memory-safe regardless, since every offset is clamped to the tag's bounds) — LRCLIB is this feature's primary/more reliable source, so full v2.4 support wasn't worth the added complexity for a best-effort fallback
  - Added `src/services/lyrics.ts` — `resolveLyrics()`: 3 sources in priority order (a `localStorage` cache, then embedded ID3 lyrics via the command above, then LRCLIB's public API), with a pure, tested `parseLrc()` for standard `[mm:ss.xx]` timestamp parsing
  - Added `src/services/radio.ts` and `src/components/radio/RadioPanel.tsx` — station URL entry, native `<audio>` playback through the local proxy, and a live now-playing title, mounted in `App.tsx`'s header
  - Added `src/components/karaoke/KaraokeStage.tsx` — a full-bleed fixed overlay (not squeezed into the ~25%-width automix dock column — "full-bleed" read as its own mode, toggled by a header button) that follows whichever deck is playing, highlights the current synced lyric line, and lets you click a line to seek there (only when lyrics are actually synced — plain text has no meaningful timestamp). Its pure logic (`pickKaraokeDeck`, `activeLyricLineIndex`) lives in a sibling `karaokeLogic.ts` so the component file stays fast-refresh-compatible (component-only exports)
  - Added `src/theme/themes.ts` — the 4 planned skins (Midnight Slate, Pioneer Stealth, Technics Silver, Day Shift) as CSS-variable definitions plus literal-color helpers (`themeAccentColor`/`themeSurfaceColor`/`themeTextColor`) for the one `<canvas>`-drawn component in the app (`WaveformCanvas.tsx`, which can't read CSS custom properties directly and now redraws on theme change); `src/index.css` gained the 3 new `[data-theme="..."]` blocks alongside Phase 1's existing Midnight Slate one. A theme `<select>` in `App.tsx`'s header drives `useUIStore`'s already-`persist`ed `theme` field
  - Extended `tauri.conf.json`'s CSP: `media-src` now allows `http://127.0.0.1:17420` (the radio proxy) and `connect-src` allows `https://lrclib.net` (lyrics lookups)
  - Added 25 new tests (206 total): `radio.rs`'s URL parsing/ICY title extraction (6), `lyrics.rs`'s USLT frame parsing (4), `parseLrc` (6), and `KaraokeStage`'s pure deck-picking/active-line functions (9)
  - Verified: `npx tsc --noEmit`, `npm run lint`, `npm run format:check`, `npm run test`, `cargo test --manifest-path src-tauri/Cargo.toml`, `npm run tauri dev` (screenshot-verified: radio controls, Karaoke toggle, and theme selector all render correctly), and `npm run tauri build -- --bundles nsis,msi` all pass
  - **Scope note**: `ImplementationPlan.md §1`'s right-dock "Automix | Sampler | Karaoke" tab switcher was never built in any phase (Phase 5/6 only ever shipped `AutomixPanel`, with no dedicated Sampler slot-management panel) — Karaoke is reachable as its own full-screen overlay instead of a dock tab, which reads as a better fit for "full-bleed" anyway; building out the 3-tab dock switcher itself remains unaddressed scope, not something this phase silently expanded to cover

### Fixed
- `waveform.worker.ts`/`audioAnalysis.worker.ts`: both were designed around a `{file: File}` request shape, but no `File` object ever actually exists anywhere in this app's data flow — Tauri file loading goes through `readFile(path)` (returns `Uint8Array`) and native OS drag-drop (`onDragDropEvent`) gives `paths: string[]`, never a `File`. Caught proactively while wiring `analyzeTrackInBackground()` into `loadTrackToDeck()`, not via a runtime error; fixed by changing both workers' request interfaces to `{bytes: ArrayBuffer}` and extending `trackLoader.ts`'s `DecodedTrack` to expose the already-read raw bytes for reuse
